...
**新生代**：每次垃圾收集时都有大批对象死去，只有少量存活，那就选用**复制算法**，只需要付出少量存活对象的复制成本就可以完成收集。
...
1. **提高回收效率**：针对不同代采用不同的收集算法，提高整体回收效率
2. **减少停顿时间**：新生代回收频繁但快速，老年代回收较少
3. **空间利用率高**：新生代使用复制算法的优化版本，老年代不需要额外空间
