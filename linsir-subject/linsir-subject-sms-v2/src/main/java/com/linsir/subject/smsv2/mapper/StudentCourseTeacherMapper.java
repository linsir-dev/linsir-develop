...
import com.linsir.subject.smsv2.entity.SCTInfo;
import com.linsir.subject.smsv2.entity.StudentCourseTeacher;
...
public interface StudentCourseTeacherMapper {
    ...
    @Select("SELECT DISTINCT sct.term FROM linsir_subject_sms_v2.sct sct")
    public List<String> findAllTerm();

    @Select("SELECT * FROM linsir_subject_sms_v2.sct WHERE sid = #{sct.sid} AND cid = #{sct.cid} AND tid = #{sct.tid} AND term = #{sct.term}")
    public List<StudentCourseTeacher> findBySCT(@Param("sct") StudentCourseTeacher studentCourseTeacher);

    @Insert("INSERT INTO linsir_subject_sms_v2.sct (sid, cid, tid, term) VALUES (#{s.sid}, #{s.cid}, #{s.tid}, #{s.term})")
    public boolean insert(@Param("s")StudentCourseTeacher studentCourseTeacher);

    @Update("UPDATE linsir_subject_sms_v2.sct SET sct.grade = #{grade} WHERE sct.sid = #{sid} AND sct.tid = #{tid} AND sct.cid = #{cid} AND sct.term = #{term}")
    ...
    @Delete("DELETE FROM linsir_subject_sms_v2.sct WHERE sid = #{sct.sid} AND tid = #{sct.tid} AND cid = #{sct.cid}")
